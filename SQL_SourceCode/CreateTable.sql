...
-- place retired deer in hot storage for retired -  working combination
-- cold storage vertical split
...
CREATE TABLE DeerToDeer( --
    ...
    -- not FOREIGN keys allow retired deer to working deer connections
...
CREATE TABLE Sled(
    Id INT,
    SledName VARCHAR(20) , -- max 3 same name
    Registration BIT(2), -- 00 = registerd (10 | 01 ) = Avregistread, 11 = förbud
    RegionId INT,
...
CREATE TABLE SledModifications(
    SledId INT,
    ExpressLast BIT(2) NOT NULL, -- b'express last'
    LastExtraStorage INT, -- kg storage 
